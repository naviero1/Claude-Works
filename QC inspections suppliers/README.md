...
| `SH vs ATM Issues` | Historical issue profiles per tissue: SH defect categories (left) vs ATM SAP reason codes (right, 2026 + all-time), side-by-side with data bars, quality-only subtotals (planned withdrawals like 1220 Destructive Testing marked *), and an alignment read per tissue. |
