...
| Sheet | Purpose |
|---|---|
| `Slaughterhouse IQC` | The core: one product-agnostic **Inspection Log** — one row per part per harvest date, all tissue types (Pelvic 666541/506/518, Thoracic 666521, …). A **Tally Format** column distinguishes Standard sheets from Nahunta's **two-stage tally** (from 08/26: SH tech screens, then QC tech inspects — the log records the final QC outcome; stage detail in a dedicated section). Then **Totals by Tissue Type** + a grand total, a **Nahunta two-stage stage-detail** section, an **ATM Receipt Comparison** pointer, and data notes. |
