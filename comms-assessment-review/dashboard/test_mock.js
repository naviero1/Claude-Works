...
(async () => {
  ...
  check('decompose: decisions nested in topic cards', /decisions made on this topic/.test(dec) && /Line stop on supplier defect/.test(dec));
  check('decompose: venue/people footer present', /where it travels/.test(dec));
  check('decompose: follow-ups stage present', /Follow-ups/.test(dec));
  const openFoldsAll = await page.$$eval('details.topicfold[open]', ds => ds.length);
  check('decompose: all topics with needs open', openFoldsAll === 4, String(openFoldsAll));
