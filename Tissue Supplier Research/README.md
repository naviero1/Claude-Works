...
2. **Smithfield BioScience (Denison, IA)** — the large packer that qualifies via the
   organ exception (full pelvic/abdominal/thoracic harvest to spec) — but note its 2024
   humane-handling suspension.
3. **Piedmont Custom Meats (NC)** & **Acre Station (NC)** — best in-region new fits.
4. **Virginia:** **T&E Meats** and **Blue Ridge Meats** (both AWA) are the live humane picks;
   **EcoFriendly Foods** reportedly ceased operations ~2020 (verify), and **Fauquier's Finest**
   has repeat humane-handling suspensions (now CAUTION).
...
6. **APHIS/EU export:** the EU requires the *origin plant* to be APHIS-listed on EU TRACES plus
   an APHIS-VS-endorsed health certificate the exporter carries — **not** a standalone "APHIS
   certificate" from the supplier. No facility could be confirmed EU/APHIS-listed for research
   tissue. For buying direct, **Innovative Research's "research use only" labeling is a likely
   EU blocker**; Tissue Source, Smithfield BioScience, MRS and SSR are best-positioned.

## Important caveats
- Weekly volumes and certifications for small/custom plants are largely **unpublished** —
  figures marked "est." or "unknown" **must be confirmed by phone**. Nothing here is fabricated.
- **"None found" in the enforcement column means no public record — not a clean bill**; FSIS
  data was only reachable via search snippets, so airtight confirmation needs a FOIA/QER query.
- **APHIS certification could not be verified for any facility** — the top diligence item,
  especially for EU export.
