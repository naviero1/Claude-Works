...
codes  = ["D6","D1","D5","D4","D8","D2","D3","D7","D9","D10","D11"]
short  = ["Membrane dmg","Ureters n/e","Urethra breach","Susp. ligament","Bowel/colon",
          "Ureter cut","Ureter missing","Bladder def.","Blood clots","Broad ligament","Bladder neck sep."]
counts = [31,11,11,8,7,3,3,1,1,1,1]
NFEW = 4
total=sum(counts); cum=[]; run=0
for c in counts: run+=c; cum.append(run/total*100)
xlabels=[f"{c} · {s}" for c,s in zip(codes,short)]

fig, ax = plt.subplots(figsize=(13.5,6.8)); fig.subplots_adjust(bottom=0.27, top=0.87)
...
ax.set_ylim(0,34); ax.set_yticks(range(0,34,4)); ax.tick_params(colors=C_AX)
...
ax.set_title("Defect Pareto — Rejected Pelvic Blocks   (SH: Martins · 2 batches · 57 inspected · 48 fail · 78 defects)",
...
st_cnt=[61,12,3,1,1]; st_col=[C_VITAL]+[C_TAIL]*4
...
for b,c in zip(b2,st_cnt):
    axs.text(b.get_x()+b.get_width()/2,c+0.7,f"{c}  ({c/78*100:.0f}%)",ha="center",va="bottom",fontsize=9.5,fontweight="bold",color="#404040")
axs.set_ylim(0,68); axs.grid(axis="y",color=C_GRID,zorder=0,linewidth=0.8); axs.set_axisbelow(True)
for s in ("top","right"): axs.spines[s].set_visible(False)
axs.spines["left"].set_color("#BFBFBF"); axs.spines["bottom"].set_color("#BFBFBF")
axs.set_title("Where the defects are born — rollup by SUPPLIER STATION\n78% of all defects originate at Evisceration",
