...
| Path | What it is |
|---|---|
| `deliverables/From_Prompts_to_Agents_Training.pptx` | The training deck (67 slides: two evidence-map reference slides, eight numbered course-log PROMPT exercises in TYPE-THIS/WHY step format, two end-to-end walkthroughs, two requirements/questions conclusion slides). Speaker notes on every slide use the spaced template: HOW TO PRESENT / BRIDGE / TRY IT (on prompt slides) / ACRONYMS / CONTENT, one item per line. Delivered as three blocks (Foundations · The craft · Delegation), each an 80-minute core expandable to ~120. |
...
3. **Prompt engineering** — the universal anatomy drawn as the body of a prompt; seven elements taught importance-first (what each buys you, with evidence); the taxonomy field-map handout; seven techniques with lines to steal; the evidence corner; the Do/Don't/Expired 2026 playbook; the PDCA iteration habit.
