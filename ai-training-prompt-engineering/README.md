...
| Path | What it is |
|---|---|
| `deliverables/From_Prompts_to_Agents_Training.pptx` | The training deck (67 slides: two evidence-map reference slides, eight numbered course-log PROMPT exercises, two end-to-end walkthroughs). Speaker notes on every slide use the spaced v1.5 template: HOW TO PRESENT / BRIDGE / TRY IT (on prompt slides) / ACRONYMS / CONTENT, one item per line. Delivered as three blocks (Foundations · The craft · Delegation), each an 80-minute core expandable to ~120. |
...
| `notes/research/` | Sixteen research files with inline source URLs (r1–r10 + t1–t6 verified Aug 21, 2026; r11–r16 verified Sep 9, 2026: vendor trust/servers, 2026 tool landscape, assistant reputations + R1 chart data, hallucination incidents, per-element evidence, email summarization). |
| `deliverables/exercise-data/` | The v1.5 practice pack (all fictional, regenerated by `src/build_exercise_pack.py`): a 144-row supplier-delivery dataset with two planted quirks, three deliberately non-comparable supplier quotation PDFs, and a messy 10-message email thread (txt + PDF). |
...
2. **Models & tools** — the Sep-2026 landscape with an evidence-backed "known for" per assistant; the DeepSeek moment (MoE defined, R1-vs-o1 chart); servers, tiers & trust; the agent gallery (doers) + specialist shelf (makers).
3. **Prompt engineering** — the universal anatomy; seven elements taught importance-first (what each buys you, with evidence); the taxonomy field-map handout; seven techniques with lines to steal; the evidence corner; expired advice; the PDCA iteration habit.
4. **The playbook** — two end-to-end walkthroughs (AI data analysis on the practice dataset + quotation comparison; email-thread summarization incl. Copilot mechanics), two live demos, the future-plays menu, and the 13-template handout.
...
python3 build_exercise_pack.py # -> deliverables/exercise-data/ (dataset, quotations, email thread)
```
