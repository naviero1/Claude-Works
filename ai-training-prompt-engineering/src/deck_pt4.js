...
module.exports = function buildPartFour(pres, H) {
  ...
  const rc = [
    ...
    ['zap', 'The techniques & the playbook', 'specificity-with-why · examples · tags · the out · chaining · self-check · metaprompting — the Do / Don’t / Expired columns (PLAYBOOK tab).'],
  ...
  const loop = [['Gather', 'read files, search, state'], ['Act', 'one step: edit, run, query'], ['Verify', 'check result vs goal'], ['Repeat', 'adjust — or stop']];
  loop.forEach((st, i) => {
    const x = 0.55 + i * 1.62;
    ...
    s.addText(st[1], { x: x + 0.06, y: 2.26, w: 1.33, h: 0.6, align: 'center', fontFace: F.body, fontSize: 9, color: C.SLATE, margin: 0, lineSpacingMultiple: 1.0 });
  ...
  s.addText('…until the goal is met, a check fails, or a guardrail says “ask the human”', { x: 1.5, y: 3.36, w: 5.5, h: 0.45, fontFace: F.body, fontSize: 10, italic: true, color: C.MUTE, margin: 0 });
  ...
  s.addText('“Systems that independently accomplish tasks on your behalf” (OpenAI) · models that “dynamically direct their own processes and tool usage” (Anthropic) — a chatbot that answers and waits is not an agent.', { x: 7.65, y: 2.32, w: 4.85, h: 1.3, fontFace: F.body, fontSize: 11, italic: true, color: C.SLATE, margin: 0, lineSpacingMultiple: 1.1 });
  const blocks = [
    ['tool', 'Tools — the arms', 'files, shell, browsers, spreadsheets, email, code. The model requests; the harness executes.'],
    ['branch', 'MCP — the standard port', '“USB-C of AI”: one open protocol to plug tools into any agent. Linux Foundation-governed since Dec 2025; ~half a billion SDK downloads/month.'],
    ['memory', 'Memory = files', 'Context window = short-term memory; notes, logs, and git = long-term. Nothing survives unless written down — Part 1’s HANDOFF move, run by the machine, is compaction.'],
  ...
  blocks.forEach((b, i) => {
    ...
    s.addText([
      ...
      { text: b[2], options: { color: C.SLATE, fontSize: 10.8 } },
  ...
  const cmp = [
    ...
    ['Contains', 'role · task · context · format · examples', 'all of that, PLUS: mission & done · environment & file boundaries · allowed tools · phases & gates · autonomy rules · self-checks · deliverables & logs'],
  ...
  cmp.forEach((r, i) => {
    ...
    s.addText(r[1], { x: 2.6, y: y + 0.05, w: 4.5, h: 0.58, fontFace: F.body, fontSize: 10.2, color: C.SLATE, margin: 0, valign: 'middle', lineSpacingMultiple: 1.0 });
    s.addText(r[2], { x: 7.35, y: y + 0.05, w: 5.2, h: 0.58, fontFace: F.body, fontSize: 10.2, color: i === 1 ? C.TEAL_DARK : C.SLATE, margin: 0, valign: 'middle', lineSpacingMultiple: 1.0 });
  });
  s.addNotes(
  ...
  const vehicles = [
    ...
    ['refresh', 'WORKFLOW', 'The loop is frozen.', 'Known path, few tools, same steps every time — script it (n8n, Zapier, Power Automate). Cheaper, predictable, auditable. No judgment needed mid-run.', C.PANEL],
  ...
  vehicles.forEach((v, i) => {
    ...
    s.addText(v[3], { x: x + 0.24, y: 2.88, w: 3.45, h: 1.42, fontFace: F.body, fontSize: 10.8, color: C.SLATE, margin: 0, lineSpacingMultiple: 1.06 });
  });
  ...
  s.addText('Don’t chat with an agent — hand it a work order.', { x: 0.55, y: 6.88, w: 3.95, h: 0.2, align: 'center', fontFace: F.body, fontSize: 10, italic: true, color: C.MUTE, margin: 0 });
  ...
  s.addText([
    ...
    { text: 'an agent on a known path pays judgment prices for clerk work; a workflow on an unknown path is a script that breaks on the first surprise.', options: { color: C.SLATE, fontSize: 10.8 } },
  ...
  const inh = [
    ...
    ['Context', '<context> + <inputs>', '+ a per-source register with grain & trust — agents touch sources repeatedly'],
  ...
  inh.forEach((r, i) => {
    ...
    s.addText(r[1], { x: 3.1, y: y + 0.03, w: 3.45, h: 0.42, fontFace: 'Consolas', fontSize: 10, color: C.TEAL_DARK, margin: 0, valign: 'middle' });
    s.addText(r[2], { x: 6.7, y: y + 0.03, w: 5.85, h: 0.42, fontFace: F.body, fontSize: 10, color: C.SLATE, margin: 0, valign: 'middle', lineSpacingMultiple: 0.95 });
  });
  ...
  const briefBlocks = [
    ...
    ['<rules>', 'non-negotiables: never invent data, versioned outputs, assumptions logged'],
    ['<quality_bar>', 'done as a checklist the agent must satisfy — and you can audit'],
    ['<reporting>', 'exact shape of the final message: status, headline, evidence, open items'],
  ];
  briefBlocks.forEach((b, i) => {
    ...
    s.addText(b[1], { x: x + 1.98, y: y + 0.05, w: 3.85, h: 0.62, fontFace: F.body, fontSize: 10, color: C.SLATE, margin: 0, valign: 'middle', lineSpacingMultiple: 0.98 });
  });
  H.callout(s, 0.55, 6.46, 12.2, 0.64, C.AMBER_TINT, [
    ...
    { text: 'if a colleague couldn’t run the job from this brief without asking a question, the agent will guess — fix the brief, not the run. Design against all twelve; SHIP the four-section PLAN · DO · CHECK · ACT brief from Part 3, one screen long.', options: { color: C.SLATE, fontSize: 10.2 } },
  ...
  const chain = [
    ['inbox', 'Extract', 'read-only; hash & profile every input; parse with explicit types; fix nothing yet'],
    ['tool', 'Transform', 'one schema, ordered rules, each logged with rows affected; metrics as cards'],
  ...
  chain.forEach((cd, i) => {
    ...
    s.addText(cd[2], { x: x + 0.2, y: 2.9, w: 1.95, h: 1.2, fontFace: F.body, fontSize: 10, color: C.SLATE, margin: 0, lineSpacingMultiple: 1.05 });
    if (i < 4) H.arrow(s, x + 2.32, 2.9, 0.16, C.TEAL);
  });
  const gates = [
    ['GATE 1 · Definitions', 'before data is touched: the agent restates metrics, schema, assumptions — you confirm. Catches a wrong denominator for pennies.'],
  ...
  gates.forEach((g, i) => {
    ...
    s.addText([
      ...
      { text: g[1], options: { color: C.SLATE, fontSize: 10.5 } },
  ...
  const guard = [
    ['hand', 'Reversible? proceed. Irreversible? ask.', 'Scratch-folder edits are reversible. Deleting, sending, publishing, overwriting a master — the prompt must say: stop and ask first. That is the EAGERNESS DIAL: persist on reversible steps, ask before irreversible.'],
    ...
    ['alert', 'Prompt injection is real', 'Anything the agent reads — a web page, an emailed doc, a spreadsheet — can carry hostile instructions. Danger peaks when private data + untrusted content + outbound channels combine: break one leg.'],
  ...
  guard.forEach((g, i) => {
    const y = 1.62 + i * 1.0;
    H.iconRow(s, 0.55, y, 9.55, g[0], i === 2 || i === 3 ? C.RED : C.TEAL, g[1], g[2], { d: 0.5, headSize: 12.5, descSize: 10.5, h: 0.96 });
  });
  ...
  s.addText([
    { text: 'A gate in the PROMPT is a suggestion. A gate in the HARNESS is a control.', options: { bold: true, color: C.RED, fontSize: 10.2, breakLine: true, paraSpaceAfter: 4 } },
    { text: 'Singapore’s regulator, 2026: prompt guardrails “are not fail-safe.”', options: { color: C.SLATE, fontSize: 8.5, italic: true } },
  ...
  const runGrid = [
    ...
    ['DRIFT', 'A long run wanders off the goal; the middle of a huge context gets skimmed.', 're-inject the mission; compact (the HANDOFF move); stop conditions in the brief.'],
  ...
  runGrid.forEach((r, i) => {
    ...
    s.addText([
      { text: r[1], options: { color: C.SLATE, fontSize: 10.5, breakLine: true, paraSpaceAfter: 3 } },
      { text: 'Cure: ', options: { bold: true, color: C.AMBER, fontSize: 10.5 } },
      { text: r[2], options: { color: C.INK, fontSize: 10.5 } },
  ...
  layers.forEach((l, i) => {
    const y = 2.24 + i * 0.56;
    s.addShape('roundRect', { x: 9.1, y, w: 3.4, h: 0.48, rectRadius: 0.05, fill: { color: 'FFFFFF' }, line: { color: C.TEAL, width: 0.75 } });
    s.addText([
      { text: l[0] + '  ', options: { bold: true, color: C.TEAL_DARK, fontSize: 10 } },
      { text: l[1], options: { color: C.SLATE, fontSize: 9 } },
    ], { x: 9.22, y: y + 0.02, w: 3.2, h: 0.44, fontFace: F.body, valign: 'middle', margin: 0, lineSpacingMultiple: 0.95 });
    if (i < 3) s.addShape('line', { x: 10.8, y: y + 0.48, w: 0, h: 0.08, line: { color: C.TEAL, width: 1 } });
  });
  ...
  const ladder = [
    ...
    ['users', '3 · Team library', 'A shared database — Notion, SharePoint, Copilot Prompt Gallery. One tested template, everyone’s baseline.'],
    ['package', '4 · Packaged & auto-applied', 'Project instructions, Claude Skills, Gems, Copilot agents: the prompt loads itself when context matches.'],
  ...
  ladder.forEach((l, i) => {
    const y = 1.62 + i * 0.99;
    H.iconRow(s, 0.55, y, 6.9, l[0], C.TEAL, l[1], l[2], { d: 0.48, headSize: 12.5, descSize: 10.8, h: 0.95 });
  });
  s.addImage({ path: require('path').join(__dirname, 'assets', 'images', 'memory_ladder.jpg'), x: 8.15, y: 1.62, w: 4.2, h: 2.36, sizing: { type: 'cover', w: 4.2, h: 2.36 } });
  s.addShape('roundRect', { x: 8.15, y: 1.62, w: 4.2, h: 2.36, rectRadius: 0.06, fill: { type: 'none' }, line: { color: C.LINE, width: 1 } });
  s.addText('sticky note → binder → library: the same prompt, promoted', { x: 7.75, y: 4.02, w: 5.0, h: 0.26, align: 'center', fontFace: F.body, fontSize: 10, italic: true, color: C.MUTE, margin: 0 });
  H.card(s, 7.75, 4.36, 5.0, 1.26, C.TEAL_TINT);
  s.addText('The promotion trigger', { x: 8.02, y: 4.46, w: 4.5, h: 0.3, fontFace: F.head, fontSize: 12.5, bold: true, color: C.INK, margin: 0 });
  s.addText('Anthropic’s rule of thumb: explained the same task more than once? Package it — with its reference material.', { x: 8.02, y: 4.78, w: 4.5, h: 0.78, fontFace: F.body, fontSize: 11, color: C.SLATE, margin: 0, lineSpacingMultiple: 1.05 });
  H.card(s, 7.75, 5.72, 5.0, 0.9, C.PANEL);
  s.addText([
    { text: 'A familiar discipline: ', options: { bold: true, color: C.INK, fontSize: 11 } },
    { text: 'run the library like controlled work instructions — name, owner, revision, review date.', options: { color: C.SLATE, fontSize: 11 } },
  ], { x: 8.02, y: 5.8, w: 4.5, h: 0.74, fontFace: F.body, valign: 'middle', margin: 0, lineSpacingMultiple: 1.05 });
  s.addNotes(
  ...
  const stores = [
    ...
    ['Any tool', 'The team library: name · purpose · template · owner · version + change note · models tested · last tested · example output · status'],
    ['Agentic (repo)', 'CLAUDE.md / AGENTS.md at the root · briefs & skills in versioned folders · reviewed like code — this repo is the worked example'],
  ];
  stores.forEach((st, i) => {
    const y = 1.62 + i * 0.82;
    ...
    s.addText(st[1], { x: 2.8, y: y + 0.06, w: 9.7, h: 0.6, fontFace: F.body, fontSize: 10.8, color: C.SLATE, margin: 0, valign: 'middle', lineSpacingMultiple: 1.0 });
  });
  H.callout(s, 0.55, 6.56, 12.2, 0.5, C.PANEL, [
    { text: 'Naming collision warning: ', options: { bold: true, color: C.INK, fontSize: 10.8 } },
    { text: '“Skills” means a packaged folder at Anthropic but a saved prompt in Chrome’s Gemini — check which one a colleague means.', options: { color: C.SLATE, fontSize: 10.8 } },
  ], { iconName: 'help', iconFill: C.SLATE, size: 10.8, line: C.LINE });
  s.addNotes(
  ...
  H.bullets(s, 0.9, 2.38, 5.4, 4.1, [
    ...
    { t: 'EVAL LITE before “approved”: three gold cases — typical · edge · should-abstain — scored with G3; a new version must beat the current; re-run on model upgrades', b: true },
