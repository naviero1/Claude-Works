...
], note='Source: Packaging_Change_Thread.txt (paste it into any approved assistant). Native path, verified Sep 2026 (notes/research/r28): open the thread in Outlook and use "Summary by Copilot" (numbered citations); then run THE PROMPT ABOVE in the Copilot chat pane, scoped to that same conversation — that is where the custom prompt goes after the built-in summary. Availability depends on your organization\'s Copilot license; the fallback that always works is pasting Packaging_Change_Thread.txt with the same prompt into any approved assistant. Never practice on real confidential threads.')
