...
```bash
cd src
node deck_main.js                    # long-format deck -> references/From_Prompts_to_Agents_Training.pptx
python3 build_course_pdf.py          # course PDF -> deliverables/ (content: src/assets/course_content.json)
python3 build_cheatsheet.py          # cheat sheet PDF -> deliverables/From_Prompts_to_Agents_Cheat_Sheet.pdf
python3 build_elements_guide.py      # ELEMENTS field-guide PDF -> references/
python3 build_builder.py             # Template Creator HTML -> deliverables/ (validates taxonomy)
python3 build_taxonomy_pdf.py        # taxonomy reference PDF -> references/
python3 build_configurator_xlsx.py   # configurator XLSX -> references/ — then ALWAYS recalc:
python3 <xlsx-skill>/scripts/recalc.py ../references/Prompt_Template_Configurator.xlsx
...
- **Part 4 (12 slides):** untouched originals + owner-requested walkthroughs: AI data
  ...
  (`src/build_exercise_pack.py` → `references/exercise-data/`: seeded 144-row dataset
